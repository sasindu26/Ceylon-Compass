...
app.use("/api/locations", (req, res, next) => {
  console.log(
    `[LOCATIONS API] ${req.method} ${req.url} - Headers:`,
...
mongoose
  ...
  .then(() => {
    ...
    cleanupPastEvents()
      .then((result) => {
        console.log(
          `Initial cleanup completed. Deleted ${result.deleted} past events.`,
        );
      })
      .catch((err) => {
    ...
    setInterval(
      () => {
        cleanupPastEvents()
          .then((result) => {
            console.log(
              `Scheduled cleanup completed. Deleted ${result.deleted} past events.`,
            );
          })
          .catch((err) => {
