...
router.post("/", auth, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("✓ Event submission email sent to:", req.user.email);
    } catch (emailError) {
...
router.get("/pending", auth, async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Not authorized to view pending requests" });
    }
...
router.post("/:id/accept", auth, async (req, res) => {
  try {
    ...
    const eventReq = await EventReq.findById(req.params.id).populate(
      "createdBy",
      "username email",
    );
    ...
    console.log(
      "Attempting to create new event with data:",
    ...
    const userEmail =
      eventReq.createdBy && eventReq.createdBy.email
        ? eventReq.createdBy.email
        : null;

    const userId =
      eventReq.createdBy && eventReq.createdBy._id
...
router.post("/:id/reject", auth, async (req, res) => {
  try {
    ...
    console.log(
      "Event request deleted successfully:",
      deleteResult ? "Yes" : "No",
    );
