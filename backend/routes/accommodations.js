...
router.get("/", async (req, res) => {
  try {
    const { country, city, showAll, minPrice, maxPrice, priceRange } =
      req.query;
  ...
  try {
    ...
    if (
      ...
      (!accommodation.createdBy ||
        accommodation.createdBy.toString() !== userId.toString())
    ) {
      return res
        .status(403)
    ...
    if (
      ...
      typeof updatedAccommodation.location === "object"
    ) {
      if (
        !updatedAccommodation.location.coordinates &&
        (updatedAccommodation.location.latitude ||
          updatedAccommodation.location.longitude)
      ) {
        updatedAccommodation.location = {
          type: "Point",
          coordinates: [
...
router.get("/admin/pending", auth, async (req, res) => {
  try {
    ...
    if (userRole !== "admin") {
      return res
        .status(403)
        .json({ message: "Not authorized to view pending accommodations" });
    }

    const accommodations = await Accommodation.find({ status: "pending" }).sort(
      { createdAt: -1 },
    );
    res.json(accommodations);
  } catch (error) {
