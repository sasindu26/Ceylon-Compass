...
router.post("/:id/accept", auth, async (req, res) => {
  try {
    ...
    const restaurantReq = await RestaurantReq.findById(req.params.id).populate(
      "createdBy",
      "username email",
    );
    ...
    try {
      const mainImage =
        restaurantReq.images && restaurantReq.images.length > 0
      ...
      console.log(
        "Creating new restaurant with data:",
        JSON.stringify(newRestaurantData, null, 2),
      );
      ...
      console.log(
        ...
        Object.keys(restaurantSchema)
          ...
          .map((key) => ({
            field: key,
            type: restaurantSchema[key].type?.name || "unknown",
          })),
      );
      ...
      if (restaurantSchema.status) {
        ...
        console.warn(
          "Status field does not exist in Restaurant schema - this could cause issues",
        );
      }
      ...
      console.log(
        "New restaurant saved successfully with ID:",
        newRestaurant._id,
      );
    } catch (saveError) {
    ...
    const userEmail =
      restaurantReq.createdBy && restaurantReq.createdBy.email
        ? restaurantReq.createdBy.email
        : null;

    const userId =
      restaurantReq.createdBy && restaurantReq.createdBy._id
...
router.post("/:id/reject", auth, async (req, res) => {
  try {
    ...
    console.log(
      "Restaurant request deleted successfully:",
      deleteResult ? "Yes" : "No",
    );
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    if (
      req.user.role !== "admin" &&
      restaurantReq.createdBy._id.toString() !== req.user._id.toString()
    ) {
