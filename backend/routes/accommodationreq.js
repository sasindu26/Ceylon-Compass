...
router.post("/", auth, async (req, res) => {
  try {
    ...
    console.log(
      "Creating accommodation request with data:",
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    console.log(
      "Fetched accommodation requests count:",
      accommodationReqs.length,
    );
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    const accommodationReq = await AccommodationReq.findById(
      req.params.id,
    ).populate({
      path: "createdBy",
      select: "username email",
    ...
    if (!accommodationReq) {
      return res
        .status(404)
        .json({ message: "Accommodation request not found" });
    }
...
router.post("/:id/accept", auth, async (req, res) => {
  ...
  try {
    ...
    console.log("Location data for new accommodation:", {
      country,
      city,
      address,
    });
    ...
    const newAccommodation = new Accommodation({
      ...
      email:
        accommodationReq.createdBy && accommodationReq.createdBy.email
          ? accommodationReq.createdBy.email
          : "no-email@example.com",
      website: "",
      createdBy:
        accommodationReq.createdBy && accommodationReq.createdBy._id
    ...
    if (userId) {
      try {
        ...
        if (userEmail) {
          ...
          console.log(
            "No valid email found for user, skipping email notification",
          );
        }
      } catch (notificationError) {
        console.error(
          "Error sending notification or email:",
          notificationError,
        );
      }
    } else {
...
router.post("/:id/reject", auth, async (req, res) => {
  try {
    ...
    const accommodationRequestCopy = JSON.parse(
      JSON.stringify(accommodationReq),
    );
    ...
    try {
      ...
      console.error(
        "Error sending rejection notification or email:",
        notificationError,
      );
    }

    const deleteResult = await AccommodationReq.findByIdAndDelete(
      req.params.id,
    );
    console.log(
      "Accommodation request deleted successfully:",
      deleteResult ? "Yes" : "No",
    );
